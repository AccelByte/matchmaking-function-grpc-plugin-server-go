...
RUN go build -v -modcacherw -o /output/$TARGETOS/$TARGETARCH/matchmaking-function-grpc-plugin-server-go .
...
# Copy build
COPY --from=builder /output/$TARGETOS/$TARGETARCH/matchmaking-function-grpc-plugin-server-go matchmaking-function-grpc-plugin-server-go

...
function clean_up()
{
  ...
  curl -X DELETE "${AB_BASE_URL}/match2/v1/namespaces/$AB_NAMESPACE/match-pools/${DEMO_PREFIX}_pool" \
      -H "Authorization: Bearer $ACCESS_TOKEN"

  echo Deleting rule sets ...

  curl -X DELETE "${AB_BASE_URL}/match2/v1/namespaces/$AB_NAMESPACE/rulesets/${DEMO_PREFIX}_ruleset" \
      -H "Authorization: Bearer $ACCESS_TOKEN"

  echo Deleting session template ...

  curl -X DELETE "${AB_BASE_URL}/session/v1/admin/namespaces/$AB_NAMESPACE/configurations/${DEMO_PREFIX}_template" \
      -H "Authorization: Bearer $ACCESS_TOKEN"
}

echo Logging in client ...

ACCESS_TOKEN="$(curl -s ${AB_BASE_URL}/iam/v3/oauth/token \
...
curl -s "${AB_BASE_URL}/session/v1/admin/namespaces/$AB_NAMESPACE/configuration" \
    -H "Authorization: Bearer $ACCESS_TOKEN" \
...
curl -s "${AB_BASE_URL}/match2/v1/namespaces/$AB_NAMESPACE/rulesets" \
    -H "Authorization: Bearer $ACCESS_TOKEN" \
    -H 'Content-Type: application/json' \
...
curl -s -X DELETE "${AB_BASE_URL}/match2/v1/namespaces/$AB_NAMESPACE/match-functions/${DEMO_PREFIX}_function" \
    -H "Authorization: Bearer $ACCESS_TOKEN"

curl -s "${AB_BASE_URL}/match2/v1/namespaces/$AB_NAMESPACE/match-functions" \
    -H "Authorization: Bearer $ACCESS_TOKEN" -H 'Content-Type: application/json' \
    -d "{\"match_function\":\"${DEMO_PREFIX}_function\",\"url\":\"$GRPC_SERVER_URL\"}"

echo Creating match pool ...

curl -s "${AB_BASE_URL}/match2/v1/namespaces/$AB_NAMESPACE/match-pools" \
    -H "Authorization: Bearer $ACCESS_TOKEN" -H 'Content-Type: application/json' \
...
for PLAYER_NUMBER in $(seq $NUMBER_OF_PLAYERS); do
  ...
  MATCH_TICKET_ID="$(curl -s "${AB_BASE_URL}/match2/v1/namespaces/$AB_NAMESPACE/match-tickets" \
  ...
  curl -X DELETE "${AB_BASE_URL}/iam/v3/admin/namespaces/$AB_NAMESPACE/users/$USER_ID/information" \
